...
int llnds(vector<int> &v)
{
    ...
    vector<int> lisCurr;// length of lis ending a ith element
    for (auto &x : v)
    {
        ...
        // it+1 gives the length of llnds till current element
        lisCurr.push_back(it+1);
    }
    return lis.size();
...
public:
    ...
    // to match two string
    // bool match(string &a,string& b,int ia,int ja,int ib,int jb){
    //     ia++;ja++;ib++;jb++; // 0-indexing to 1-indexing
    //     int lena=ja-ia;
    //     ll numa = (hasha[ja]-hasha[ia-1]*ppow[lena+1]%mod+mod)%mod;
    //     int lenb = jb-ib;
    //     ll numb = (hashb[jb]-hashb[ib-1]*ppow[lenb+1]%mod+mod)%mod;
    //     return numa==numb;
    // }
    ...
    {
        int len = (r - l + 1) / 2;
        // this fails for single length, need to update
...
int Dijkstra_min_path(int src, int dest, int n, vector<pair<int, int>> V[])
{
    ...
    while (!pq.empty())
    {
        ...
        // if u was closer through some other path
...
//  **************** Lowest Common Ancestors using Binary Lifting  ********************
// variables used for lowest common ancestor
vector<vector<int>> adj; // adjacency list for graph
vector<vector<int>> up;  // up matrix to store ancestors
vector<int> tin, tout;   // in and out times of each node in dfs
int timer;               // timer
...
{
   tin[curr] = ++timer;
   // updating immediate ancestor of curr as parent
   up[curr][0] = par;
   // updating all other ancestors 2^ith ancestor is 2^(i-1)th ancestor of 2^(i-1)th ancestor
   for (int i = 1; i <= L; ++i)
...
// Eulerian Circuit: A Eulerian path that is a cycle
// A eulerian circuit exists if and only if indegree of each node is equal to outdegree of that node
// In case eulerian circuit doesn't exists, eulerian path can exists if exactly one node have outdeg = indeg +1 (starting node)
// and exactly one node having indeg = outdeg + 1 (ending node)
// For undirected, each node should have even degree or exactly two nodes should have odd degree
// Proof : https://www.youtube.com/watch?v=wC99T3aVDKQ - G should be connected 
// on each visit for a node, an edge requires for entering and an edge for exiting the node and as these
// edges are not repeated, there are same number of in and out degrees for a node

void euler(unordered_map<int,stack<int>> &adj,vector<vector<int>> &ans,int u)
{
    while(!adj[u].empty())
    {
        int v = adj[u].top();
...
// the reverse of ans will be eulerian path
...
/// CP Tips
// 1) For large numbers, log2(x) may not give accurate answer, the result may sometimes larger than log2(x)
//              check the power of the obtained value is less than x, if not decrease answer, if((1ll<<t)>x) t--;
//              better to avoid inbuilt functions in such cases, ex: sqrt(x)
// 
// 2) sometime check if long long is missing where required on frequent answers
//
//